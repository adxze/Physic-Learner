...
@export (ParticleProcessMaterial) var droplets_material_path
...
func create_water_block(): #criação da água
	...
	#sinais
	area.connect("body_entered", Callable(self, "body_emerged"))
	area.connect("body_exited", Callable(self, "body_not_emerged"))
...
func body_emerged(body): #acionado quando um corpo da classe RigidBody2D colide com a area da água
	if body is RigidBody2D:
		...
		if droplets_material_path != null:
			var droplets = GPUParticles2D.new()
...
func _draw(): #desenho de polígono para visualização pelo editor
	...
	if Engine.is_editor_hint():
		vecs = PackedVector2Array([Vector2(0, -ALTURA), Vector2(LARGURA, -ALTURA), Vector2(LARGURA, 0), Vector2(0, 0)])
		color = PackedColorArray([Color(1.0, 1.0, 1.0, 0.4), Color(1.0, 1.0, 1.0, 0.4), Color(1.0, 1.0, 1.0, 0.4), Color(1.0, 1.0, 1.0, 0.4)])
